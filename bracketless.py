...
def rich_repr_from_any(v):
    ...
    if type(v) == list:
        return (
            [("[", 0)]
            + indent_rich_repr(flatten_list([rich_repr_from_any(elem) for elem in v]))
            + [("]", 0)]
        )
    if type(v) == tuple:
        return (
    ...
    if type(v) == dict:
        return (
            ...
            + indent_rich_repr(
                flatten_list(
                    [
                        [(f"{key}:", 0)] + indent_rich_repr(rich_repr_from_any(value))
    ...
    if type(v) == Node:
        return [(f"Node.{v.type.name}:", 0)] + indent_rich_repr(
            rich_repr_from_any(v.value)
        )
    if type(v) == ExecutionEnvironment:
        return [("ExecutionEnvironment:", 0)] + indent_rich_repr(
            rich_repr_from_any(v.env)
        )
    raise Exception(f"Could not format value of type {type(v)}")


def string_from_rich_repr(rich_repr):
    return "\n".join(["  " * indentation + line for (line, indentation) in rich_repr])
...
class Node:
    ...
    class Type(enum.Enum):
        ...
        def is_iterable(node_type):
            return node_type in [Node.Type.String, Node.Type.List]

        def is_number(node_type):
            return node_type in [Node.Type.Integer, Node.Type.Float]
