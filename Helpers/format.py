...
def run(lst):
    ...
    print(' '.join(lst))
    return subprocess.run(lst, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf-8", cwd=path)
...
for i in range(len(formatted_commits))[::-1]:
    ...
    run(["git", "push", "--force", "upstream", f"formatted-master~{i}:formatted-master"])
