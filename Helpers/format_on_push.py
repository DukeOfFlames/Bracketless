...
def run(lst):
    print(" ".join(lst))
    return subprocess.run(lst, stdout=subprocess.PIPE, stderr=subprocess.PIPE, encoding="utf-8")
...
run(["git", "push", "--force", "origin", f"{new_formatted_master_commit}:formatted-master"])
